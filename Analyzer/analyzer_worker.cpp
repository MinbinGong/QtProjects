...
bool analyzer_worker::execute(QWidget *parent, const QString &reference, const QImage &target)
{
    ...
    cv::Mat image_src = cv::imread(reference.toStdString());
...
{
    in_image = in_image.convertToFormat(QImage::Format_RGB888);
    return cv::Mat(in_image.height(), in_image.width(), CV_8UC(3), in_image.bits(), in_image.bytesPerLine());
}
